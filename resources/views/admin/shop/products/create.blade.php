<x-admin.layout>
    <x-admin.breadcrumb title='Create Product' :links="[
                    ...
                    <div class="form-group">
                        ...
                        <div class="small text-info">
                            ...
                            <div><b>*</b> Image should be in
                                {{ config('ashop.images.products.width', 800) . ' x ' . config('ashop.images.products.height', 900) }}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-4 col-6">
                    <div class="form-group">
                        ...
                        <input type="datetime-local" name="deal_expiry" class="form-control"
                            value="{{ old('deal_expiry') }}" min="{{ date('Y-m-d') . 'T' . date('H:i') }}">
                    </div>
                </div>
                ...
                <div class="col-md-3 col-6">
                    <div class="form-group">
                        ...
                        <select name="featured" class="form-control" required>
                            <option value="0" {{ old('featured') == '0' ? 'selected' : '' }}>Not Featured
                            </option>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <select name="checkout_type" class="input-group-text" required>
                                <option value="checkout" @selected(old('checkout_type') == 'checkout')>Checkout</option>
                                <option value="query" @selected(old('checkout_type') == 'checkout')>Query</option>
                                <option value="external_url" @selected(old('checkout_type') == 'checkout')>External url</option>
                            </select>
                            <input type="url" name="external_url" id="external_url" class="form-control"
                                value="{{ old('external_url') }}" placeholder="External url">
                        </div>
                    </div>
                </div>
            </div>
    ...
    @push('scripts')
        <script>
            var imageRatio = {{ config('ashop.products.images.width', 800) . '/' . config('ashop.products.images.height', 900) }};
            var previewImg = {
